...
# Step 7: GitHub runner setup instructions
print_step "Step 7: GitHub runner setup"

echo ""
print_warning "Self-hosted runner setup required"
echo ""
echo "Choose a host for the GitHub runner (recommendations):"
echo "  - node005 (Proxmox host) - has network access to all VMs"
echo "  - Dedicated VM with network access to homelab"
echo "  - Docker container with proper network routing"
echo ""
echo "Runner setup commands:"
echo "----------------------"
...
echo "# https://github.com/$(gh repo view --json owner,name -q '.owner.login + \"/\" + .name')/settings/actions/runners/new"
echo ""
echo "./config.sh --url https://github.com/$(gh repo view --json owner,name -q '.owner.login + \"/\" + .name' 2>/dev/null || echo 'YOUR_ORG/kubeadm_ansible') --token YOUR_TOKEN"
echo ""
...
echo ""
echo "📋 Next Steps:"
echo "   1. Set up self-hosted GitHub runner"
